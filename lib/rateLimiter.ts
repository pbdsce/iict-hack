...
export function createRateLimitMiddleware(limiterType: keyof typeof rateLimiters) {
  return async function rateLimitMiddleware(request: NextRequest | Request) {
    ...
    try {
      ...
      // Rate limit exceeded
      const errorRes = rateLimiterRes as { remainingPoints?: number; msBeforeNext?: number };
...
export const teamRegistrationRateLimit = createRateLimitMiddleware('teamRegistration');
export const collegeCreationRateLimit = createRateLimitMiddleware('collegeCreation');
...
export async function checkRateLimit(limiterType: keyof typeof rateLimiters, request: NextRequest | Request) {
  const limiter = rateLimiters[limiterType];
  ...
  try {
    ...
    if (rateLimiterRes && rateLimiterRes.consumedPoints !== undefined && rateLimiterRes.consumedPoints > 0) {
      return {
        allowed: true,
